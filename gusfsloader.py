...
class GusFSLoader(object):
    ...
    def reload_templates(self):
        ...
        assert os.path.isfile(
            properties_file), "%s must be a file" % properties_file
        ...
        assert os.path.isdir(
            templates_path), "%s must be a dir" % templates_path
        ...
        assert os.access(
            rendered_path, os.W_OK), "%s must be writable" % rendered_path
        ...
        for page_type, info in list(self.page_types.items()):
            ...
            page_template = os.path.join(
                templates_path,
                "%s.mustache" %
                page_type)
            assert os.path.isfile(
                page_template), "%s must be a file" % page_template
            ...
            if 'indices' in info:
                ...
                for index_name, index_info in list(info['indices'].items()):
                    page_template = os.path.join(
                        templates_path, "%s-index-%s.mustache" %
                        (page_type, index_name))
                    assert os.path.isfile(
                        page_template), "%s must be a file" % page_template
                    with open(page_template, 'r') as f:
                        self.gus.set_index_template(
                            page_type, index_name, f.read())

        layout_template = os.path.join(templates_path, "layout.mustache")
        assert os.path.isfile(
            layout_template), "%s must be a file" % layout_template
        with open(layout_template, 'r') as f:
            self.gus.set_site_template(f.read())

    def render_pages(self):
        for page_path, page_contents in list(
                self.gus.render_renderables().items()):
            ...
            for dirname, dirnames, filenames in os.walk(
                ...
                for filename in filenames:
                    ...
                    if re.match(handler['regex'], filename):
                        ...
                        rel_path = re.sub(
                            self.handled_assets_path, '', filename)
                        ...
                        dest_name = dest_name_noext + \
                            "." + handler['output-ext']
                        print(
                            "Running %s on %s with output %s" %
                            (handler['program']['path'], filename, dest_name))
    ...
    # It then removes the tmp folder and sets up another one for use if we
    # render again
    def finalize(self):
