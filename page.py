        ...
        if self.metadata or self.metadata == {}:
            ...
            if not 'date' in self.metadata:
                ...
                self.metadata['date'] = datetime.strptime(
                    self.metadata['date'], "%Y-%m-%d")
                self.metadata[
                    'date-year'] = str(self.metadata['date'].timetuple()[0])
                self.metadata[
                    'date-month'] = "%02d" % self.metadata['date'].timetuple()[1]
                self.metadata[
                    'date-day'] = "%02d" % self.metadata['date'].timetuple()[2]
                self.metadata['date'] = self.metadata[
                    'date'].strftime("%Y-%m-%d")
